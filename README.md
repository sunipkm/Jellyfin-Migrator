...
* Most likely: Folders that used to be different are merged into the same new folder by your path replacement rules. This can be intentional if e.g. you had media files spread across different drives and now move/copy them into the same folder. If that's indeed the case, you can ignore this message. 
...
Where things get interesting are the `metadata`, `plugin` and the other remaining folders from `$JF_DATA_DIR`: Looking at the new installation, we find them at `mnt/user/appdata/jellyfin/data` - which is once again mapped by Docker to `/config/data`.
