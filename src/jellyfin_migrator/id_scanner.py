...
def convert_ancestor_id(id: str):
    # Group by bytes
    id = [id[i: i+2] for i in range(0, len(id), 2)] # type: ignore
...
def sid2did(id): return "-".join([id[:8],
                                  id[8:12], id[12:16], id[16:20], id[20:]])
...
def load_ids(library_db: str):
    ...
    cur = con.cursor()
    id_replacements_bin = [x[0] for x in cur.execute(
...
def load_db_tables_columns(path_to_db):
    ...
    table_info = {n: [x[0] for x in cur.execute(
        f"SELECT name FROM PRAGMA_TABLE_INFO('{n}')")] for n in table_names}
...
def load_all_rows(path_to_db):
    ...
    for table, columns in table_info.items():
        for column in columns:
            col_values = {x[0] for x in cur.execute(
                f"SELECT `{column}` FROM `{table}`") if x[0]}
            if not col_values:
                continue
...
def get_id_candidates(s):
    ...
    if type(s) is bytes:
        result = "".join(
            chr(c) if c in b"0123456789abcdef-" else " " for c in s)
    elif type(s) is str:
...
def jellyfin_id_scanner():
    ...
    for i, job in enumerate(jobs):
        col_values = job[2]
        with Pool() as p:
            col_values = [x for x in p.imap_unordered(
                get_id_candidates, col_values, chunksize=64) if x[1]]
        jobs[i] = (job[0], job[1], col_values, ids)

    check_embedded_id_types(jobs[i]) # type: ignore
    with Pool() as p:
    ...
    results = [[x[i].ljust(lengths[i] + 1)
                for i in range(len(x))] for x in results]
    for x in results:
        print(*x)
